...
function getBloqueAuditorias(e) {
    ...
    selector = document.getElementById('tipo_auditoria');
    datos_generales = document.getElementById('datos_generalesControl');
    bloque_obs = document.getElementById('obs_control');
    boton_control = document.getElementById('boton_control');
    ...
    if (id_auditoria != '') {
        fetch(GET_BASE_URL() + "/api/auditorias/getBloqueAud/" + id_auditoria)
            ...
            .then((data) => {
                ...
                bloque_obs.style.display = "block"

                boton_control.style.display = "block"
            });
    } else {
        text_aud_title.textContent = '';
        bloque_rtas_container.innerHTML = '';

        datos_generales.style.display = "none"
        bloque_obs.style.display = "none"
        boton_control.style.display = "none"
