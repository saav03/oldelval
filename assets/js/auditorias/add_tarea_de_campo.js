...
const bloque_rtas_container_t = document.getElementById('bloque_respuestas_container_t'); /* (Contenedor de preguntas) */
const text_aud_title_t = document.getElementById('text-aud_title_t'); /* (Contenedor de preguntas) */

/**
 * Escucha por un cambio en el selector del tipo de auditorías y en base a eso
 * genera el bloque de preguntas.
 */
function getBloqueAuditorias_t(e) {


    selector_t = document.getElementById('tipo_auditoria_t');
    datos_generales_t = document.getElementById('datosgenerales_tarea');
    bloque_obs_t = document.getElementById('obs_tarea');
    boton_tarea = document.getElementById('boton_tarea');
    ...
    if (id_auditoria != '') {
        fetch(GET_BASE_URL() + "/api/auditorias/getBloqueAud/" + id_auditoria)
            ...
            .then((data) => {
                ...
                obs_tarea.style.display = "block"

                boton_tarea.style.display = "block"

            });
    } else {
        text_aud_title_t.textContent = '';
        bloque_rtas_container_t.innerHTML = '';
        datosgenerales_tarea.style.display = "none"
        bloque_obs_t.style.display = "none"
        boton_tarea.style.display = "none"
        vehicular_datos_generales.style.display = "none"
    }
...
function _generar_bloque_preguntas_t(datos) {

    text_aud_title_t.textContent = datos['bloque'][0].nombre;
    ...
    for (let i = 0; i < datos['bloque']['bloque_preguntas'].length; i++) {
        ...
        let divSubtitleAud = el('div.div-subtitle_aud_t');
        let h6 = el('h6.subtitle_aud_t', datos['bloque']['bloque_preguntas'][i].nombre);
        ...
        for (let j = 0; j < bloque_preguntas.length; j++) {
            ...
            // -- Botones Toggle
            let btnsToggle = _generar_btns_toggle_t(datos['bloque']['bloque_preguntas'][i].id, bloque_preguntas[j].id, bloque_preguntas[j].pregunta);
            ...
            let txtArea = el('textarea.form-control sz_inp comment_accion', {
                ...
                name: "comentarios_preguntas_t[" + bloque_preguntas[j].id + "]",
            });
        ...
        /* Último mount */
        mount(bloque_rtas_container_t, divRowContainer);
    }
 ...
 */
function _generar_btns_toggle_t(aux, id_pregunta, pregunta) {
    ...
    input = el('input.btn-check verde_check', {
        type: 'radio',
        name: "bloque_respuestas_t[" + aux + "][respuestas][" + id_pregunta + "][rta]",
        value: '1',
    ...
    input_pregunta = el('input', {
        type: 'hidden',
        name: "bloque_respuestas_t[" + aux + "][respuestas][" + id_pregunta + "][pregunta]",
        value: pregunta,
    });
