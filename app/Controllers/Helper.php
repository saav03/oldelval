...
class Helper extends Controller
{
    ...
    public function sendMailTarjeta($datos, $tipo_mail)
    {
        ...
        $config["SMTPHost"] = 'smtp.mail.us-east-1.awsapps.com';
        $config['mailType'] = 'html';
        $config["SMTPUser"] = 'no-reply@blistertechnologies.com';
        $config["SMTPPass"] = 'KqynC4A>SgR[p&bE';
        ...
        $email->setFrom('no-reply@blistertechnologies.com', 'OLDELVAL');
        $email->setBCC($correos);
        $email->setSubject('Oldelval | ' . $subject);
        $email->setMessage($message);
        $email->send();

        // ¡Credenciales Antiguos de CPANEL!
        // $config['protocol'] = 'smtp';
