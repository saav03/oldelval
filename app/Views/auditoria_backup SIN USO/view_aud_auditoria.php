...
<title>OLDELVAL - Inspección de Auditoría</title>
<div class="container">
    <div class="row" style="box-shadow: 0px 0px 10px 10px rgba(250,250,250,1);">
        <div class="col-md-12"
            style="border-radius: 10px;background: #fcfdff;border-bottom: 2px solid #f1f1f1;border-top: 2px solid #f1f1f1;">
            <h5 class="text-center" style="padding: 10px;color: #84baeb;font-size: 24px;letter-spacing: 1px;">
                Inspección de Auditoría N° <?= $auditoria['id_auditoria']; ?>
            </h5>
            ...
            <div class="row">
                ...
                <div class="col-xs-12 col-md-3 text-center">
                    <p><b>Usuario Carga</b>:
                        <?= isset($auditoria['usuario_carga']) ? $auditoria['usuario_carga'] : '-'; ?>
                    </p>
                </div>
                <div class="col-xs-12 col-md-3 text-center">
                    <p><b>Fecha de Carga</b>:
                        <?= isset($auditoria['fecha_carga_format']) ? $auditoria['fecha_carga_format'] : '--/--/----'; ?>
                    </p>
                </div>
                <div class="col-xs-12 col-md-3"></div>
            </div>
            ...
            <div class="row">
                <div class="col-xs-12 col-md-4 text-center">
                    ...
                    <input type="text" class="form-control sz_inp"
                        value="<?= isset($auditoria['contratista']) ? $auditoria['contratista'] : 'No Aplica'; ?>"
                        readonly>
                </div>
                <div class="col-xs-12 col-md-4 text-center">
                    <label class="sz_inp fw-semibold text-center">Supervisor Responsable</label>
                    <input type="text" class="form-control sz_inp"
                        value="<?= isset($auditoria['supervisor_responsable']) ? $auditoria['supervisor_responsable'] : 'No Aplica'; ?>"
                        readonly>
                </div>
                <div class="col-xs-12 col-md-4 text-center">
                    <label class="sz_inp fw-semibold text-center">Cantidad del Personal</label>
                    <input type="number" class="form-control sz_inp text-center"
                        value="<?= isset($auditoria['cant_personal']) ? $auditoria['cant_personal'] : 'No Aplica'; ?>"
                        readonly>
                </div>
                <!-- <div class="col-xs-12 col-md-3 text-center">
                    <label class="sz_inp fw-semibold text-center">N°Informe</label>
                    <input type="number" class="form-control sz_inp text-center"
                        value="<?= isset($auditoria['num_informe']) ? $auditoria['num_informe'] : 'No Aplica'; ?>"
                        readonly>
                </div> -->
            </div>
            <div class="row mt-2">
                <div class="col-xs-12 col-md-3 text-center">
                    ...
                    <input type="text" class="form-control sz_inp"
                        value="<?= isset($auditoria['proyecto']) ? $auditoria['proyecto'] : 'No Aplica'; ?>" readonly>
                </div>
                <div class="col-xs-12 col-md-3 text-center">
                    <label class="sz_inp fw-semibold text-center">Módulo</label>
                    <input type="text" class="form-control sz_inp"
                        value="<?= isset($auditoria['modulo']) ? $auditoria['modulo'] : 'No Aplica'; ?>" readonly>
                </div>
                <div class="col-xs-12 col-md-3 text-center">
                    <label class="sz_inp fw-semibold text-center">Estación de Bombeo</label>
                    <input type="text" class="form-control sz_inp"
                        value="<?= isset($auditoria['estacion']) ? $auditoria['estacion'] : 'No Aplica'; ?>" readonly>
                </div>
                <div class="col-xs-12 col-md-3 text-center">
                    <label class="sz_inp fw-semibold text-center">Sistema de Oleoducto</label>
                    <input type="text" class="form-control sz_inp"
                        value="<?= isset($auditoria['sistema']) ? $auditoria['sistema'] : 'No Aplica'; ?>" readonly>
                </div>
            </div>
        </div>
    </div>
                        ...
                        <div style="width: 95%!important; margin: 0 auto;">
                                ...
                                <div class="col-xs-12 col-md-4 text-end">
                                    <div class="pb-2 pt-2">
                                        <div class="btn-group btn-group-toggle" role="group"
                                            ...
                                            <input type="radio" class="btn-check verde_check" value="1" autocomplete="off"
                                                <?= $pregunta['respuesta'][0]['rta'] == '1' ? 'checked' : '' ?>>
                                            <label
                                                class="btn verde btnsToggle <?= $pregunta['respuesta'][0]['rta'] != '1' ? 'disabled_btn' : '' ?>">Bien</label>

                                            <input type="radio" class="btn-check rojo_check" value="0" autocomplete="off"
                                                <?= $pregunta['respuesta'][0]['rta'] == '0' ? 'checked' : '' ?>>
                                            <label
                                                class="btn rojo btnsToggle <?= $pregunta['respuesta'][0]['rta'] != '0' ? 'disabled_btn' : '' ?>">Mal</label>

                                            <input type="radio" class="btn-check amarillo_checked" value="-1" autocomplete="off"
                                                <?= $pregunta['respuesta'][0]['rta'] == '-1' ? 'checked' : '' ?>>
                                            <label
                                                class="btn amarillo btnsToggle <?= $pregunta['respuesta'][0]['rta'] != '-1' ? 'disabled_btn' : '' ?>">N/A</label>
                                        </div>
                                    </div>
                                </div>
                            </div> <!-- .pregunta_rta -->
                            ...
                            <?php if (!empty($pregunta['respuesta'][0]['comentario'])): ?>
                                <div class="row comment-question_rta">
                                    ...
                                    <textarea class="form-control sz_inp" rows="3"
                                        readonly><?= $pregunta['respuesta'][0]['comentario'] ?></textarea>
...
<section>
    <?php if ($hallazgo != null): ?>
        <?php $data['h'] = $hallazgo; ?>
        <?= view('auditoria/auditoria/aud_plan_auditoria', $data); ?>
    <?php endif; ?>
</section>
