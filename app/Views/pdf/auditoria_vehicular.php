...
<head>
    ...
    <title>Oldelval - CheckList Vehicular</title>
    <!-- Font awesome v6.2 -->
...
<body>
    <div class="footer">
        Oldelval | Auditoría N°<?= $auditoria['id_auditoria']; ?> - CheckList Vehicular <span class="pagenum"></span>
    </div>
    <!-- Encabezado -->
    <table width='100%' style="border: 1px solid #494949; border-radius: 5px;">
        ...
        <th style="width: 80%; text-align: center; color: #494949; padding-top: 5px; letter-spacing: 1px;">
            CheckList Vehicular - N°<?= $auditoria['id_auditoria']; ?>
        </th>
    </table>
    ...
    <table width="100%;" style="border: 1px solid #494949; border-radius: 5px; margin-top: 6px; font-size: 12px; text-align: center; color: #686868;">
        <thead>
            <tr>
                <th class="th_subtitle">Equipo</th>
                <th class="th_subtitle">Conductor</th>
                <th class="th_subtitle">N° Interno</th>
                <th class="th_subtitle">Marca</th>
                <th class="th_subtitle">Modelo</th>
            </tr>
        </thead>

        <tbody>
            <tr>
                <td class="td_content"><?= $auditoria['equipo'] ? $auditoria['equipo'] : '' ?></td>
                <td class="td_content"><?= $auditoria['conductor'] ? $auditoria['conductor'] : '' ?></td>
                <td class="td_content"><?= $auditoria['num_interno'] ? $auditoria['num_interno'] : '' ?></td>
                <td class="td_content"><?= $auditoria['marca'] ? $auditoria['marca'] : '' ?></td>
                <td class="td_content"><?= $auditoria['modelo'] ? $auditoria['modelo'] : '' ?></td>
            </tr>
        </tbody>

        <thead>
            <tr>
                <th class="th_subtitle_dos">Patente</th>
                <th class="th_subtitle_dos">Titular</th>
                <th class="th_subtitle_dos">Fecha</th>
                <th class="th_subtitle_dos">Hora</th>
        ...
        <tbody>
            <tr>
                <td class="td_content"><?= $auditoria['patente'] ? $auditoria['patente'] : '' ?></td>
                <td class="td_content"><?= $auditoria['titular'] ? $auditoria['titular'] : '' ?></td>
                <td class="td_content"><?= $auditoria['fecha_carga_format'] ? $auditoria['fecha_carga_format'] : '' ?></td>
                <td class="td_content"><?= $auditoria['hora'] ? $auditoria['hora'] : '' ?></td>
                <td class="td_content"><?= $auditoria['proyecto'] ? $auditoria['proyecto'] : '' ?></td>
            </tr>
        </tbody>
    ...
    <div style="border: 1px solid #494949; border-radius: 5px; margin-top: 6px;">
        ...
        foreach ($auditoria['bloque'] as $bloque) : ?>
            ...
            <!-- Preguntas Bloque -->
            <table width="100%;" style="margin-top: 6px; text-align: center; font-size: 12px; color: #686868;">
                <tbody>
                    <?php foreach ($bloque['preguntas_rtas'] as $rtas) : $i++; ?>
                        <tr>
                            <td style="text-align: left; width: 60%; font-size: 12px;">
                            ...
                            <td style="width: 28%; color: #686868;">
                                <?php

                                if ($rtas['respuesta'][0]['tipo_obs'] == 0) {
                                    echo '<em>No Aplica</em>';
                                } else {
                                    foreach ($tipo_obs_vehicular as $obs) {
                                        if ($rtas['respuesta'][0]['tipo_obs'] == $obs['id']) {
                                            echo $obs['tipo'];
                                        }
                                    }
                                }
                        ...
                        <?php if ($rtas['respuesta'][0]['comentario'] != '') : ?>
                            <tr>
                                <td colspan="4" style="font-weight: bold; letter-spacing: .3px; color: #686868; font-size: 12px;">
                                    Comentario
                                </td>
                            </tr>
                            <tr>
    ...
    <table width="100%;" style="color: #343434;border: 1px solid #494949; border-radius: 5px; margin-top: 15px; font-size: 12px; text-align: center; color: #686868; ">
        <thead style="border-bottom: 1px solid #494949;">
            <tr>
                <th style="width: 100%; border-right: 1px solid #494949; background-color: #f1f1f1; border-radius: 5px 0 0 0; color: #343434;">
        ...
        <tbody>
            <tr>
                <td style="border-right: 1px solid #494949; padding: 3px; text-align: center;">
                    <?= $auditoria['resultado_inspeccion'] == 0 ? 'No Satisfactoria' : 'Satisfactoria'; ?>
                </td>
            </tr>
        </tbody>
    </table>
        ...
        <tbody>
            <tr>
                <td style="border-right: 1px solid #494949; padding: 3px; text-align: left;">
                    <?= $auditoria['medidas_implementar'] ? $auditoria['medidas_implementar'] : 'No se agregaron medidas a implementar' ?>
                </td>
            </tr>
        </tbody>
    </table>
    ...
    <?php if (!is_null($hallazgo)) : ?>
        ...
        <table width="100%;" style="border: 1px solid #494949!important; border-radius: 5px; margin-top: 15px; font-size: 12px; text-align: center; color: #686868; ">
            ...
            <tbody>
                <tr>
                    <td style="border-right: 1px solid #494949;"><?= $hallazgo['relevo'] ? $hallazgo['relevo'] : '<em>No Aplica</em>'; ?></td>
